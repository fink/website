...
if (!$level)
  $level = 0;
$binbase = "";
for ($i = 0; $i < $level; $i++) $binbase .= "../";

// get relative directory name
if ($level > 0) {
  $parts = split("/", $SCRIPT_NAME);
  $dir = join("/", array_slice($parts, -$level-1, $level))."/";
} else {
  $dir = "";
}
...
if ($level) {
  print "<p>Directory: <a href=\"$binbase\">Root</a>";
  $parts = split("/", $dir);
  for ($i = 0; $i < $level-1; $i++) {
    print " / <a href=\"";
    for ($j = $i; $j < $level-1; $j++) {
...
{
  global $rowcolor1, $rowcolor2, $modulo, $binbase;
  ...
  print "<td align=\"center\">$link<img src=\"".$binbase."icons/$icon.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"[$iconalt]\">$linkend</td><td>$link$name$linkend</td><td>&nbsp;</td><td align=\"right\">$size</td><td>&nbsp;</td><td>$date</td><td>&nbsp;</td><td>$special</td></tr>\n";
}
...
<tr bgcolor="<? print $headrowcolor ?>">
<td><img src="<? print $binbase ?>icons/shim.gif" width="24" height="1" border="0" alt=""></td>
<td><img src="<? print $binbase ?>icons/shim.gif" width="200" height="1" border="0" alt=""></td>
<td colspan="6"><img src="<? print $binbase ?>icons/shim.gif" width="1" height="1" border="0" alt=""></td>
</tr>
