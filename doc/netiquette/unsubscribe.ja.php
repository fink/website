<?
$title = "ネチケット - 購読解除";
$cvs_author = 'Author: gecko2';
$cvs_date = 'Date: 2012/11/11 15:20:16';
$metatags = '<link rel="contents" href="index.php?phpLang=ja" title="ネチケット Contents"><link rel="prev" href="reply.php?phpLang=ja" title="投稿への返信">';


include_once "header.ja.inc";
?>
<h1>ネチケット - 4. 定期購読の解除</h1>
    
    
    <h2><a name="unsubscribing">4.1 定期購読の解除はどうしたらいいですか?</a></h2>
      
      <p>購読解除のメールはリストに送ら<b>ない</b>でください．</p>
      <ol>
        <li><a href="/lists/index.php">メーリングリスト</a> ページに行く．
		<p>注記: MacOSX 版の Internet Explorer は使わないでください．</p></li>
        <li>購読解除したいリストのリンクを辿る．</li>
        <li>ページ下部の:  
		<q>To change your subscription (set options like digest and delivery modes, 
		get a reminder of your password, or unsubscribe), enter your subscription email 
		address:</q>
		に従い，購読解除したいメールアドレスを入力し， <q>Edit Options</q> ボタンを押す．</li>
        <li>次のページの左上に購読解除がある．</li>
      </ol>
    
  
<? include_once "../../footer.inc"; ?>


