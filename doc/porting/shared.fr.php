...
$cvs_date = 'Date: 2006/01/17 08:15:22';
...
<p>Pour construire une librairie partagée, vous invoquez le compilateur avec l'option <code>-dynamiclib</code>. Voici un exemple qui permettra de mieux comprendre le processus. Nous allons construire une librairie appelée libfoo, composée des fichiers sources <code>source.c</code> et <code>code.c</code>. Le numéro de version est 2.4.5, où 2 est le numéro de révision majeure (changement d'API incompatible), 4 est le numéro de révision mineure (compatible avec un retour à une API antérieure) et 5 est le compteur de révision des bogues (on l'appelle parfois une "mini-révision", elle indique des changements totalement compatibles avec l'API). La librairie ne dépend d'aucune autre librairie partagée et sera installée dans <code>/usr/local/lib</code>.</p>
...
<p>Observez quelles sont les parties du numéro de version qui sont utilisées et où elles le sont. Lors de l'édition de lien avec cette librairie, on utilise normalement le drapeau <code>-lfoo</code>, qui donne accès au lien symbolique <code>libfoo.dylib</code>. Quel que soit le lien symbolique ou le fichier utilisé, c'est le <code>nom_d'installation</code> qui est écrit dans le programme. Cela signifie que l'on peut supprimer le lien symbolique "de plus haut niveau" (celui qui est le moins spécifique d'une version donnée) <code>libfoo.dylib</code> après compilation. Lors d'une mise à jour mineure, on change juste la cible du lien symbolique <code>libfoo.2.dylib</code> utilisé par l'éditeur de lien dynamique.</p>
