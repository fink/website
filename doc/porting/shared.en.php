...
<h1>Porting - 2. Shared Code</h1>
		...
		<h2><a name="build-lib">2.4 Building a Shared Library</a></h2>
			...
			<p>To build a shared library, you invoke the compiler driver with the <code>-dynamiclib</code> option. This is best demonstrated by a comprehensive example. We'll build a library called libfoo, composed of the source files <code>source.c</code> and <code>code.c</code>. The version number is 2.4.5, where 2 is the major revision (incompatible API change), 4 is the minor revision (backwards-compatible API change) and 5 is the bugfix revision count (some people call this the "teeny" revision, it denotes fully compatible changes). The library depends on no other shared libraries and will be installed in <code>/usr/local/lib</code>.</p>
...
When linking against this library, one would normally use the <code>-lfoo</code> flag, which accesses the <code>libfoo.dylib</code> symlink. Regardless of which symlink or file is specified, though, it is the <code>install_name</code> that is written into one's program. That means one can delete the "higher" (less version-specific) symlink <code>libfoo.dylib</code> after compiling. During a minor-revision library upgrade, one just changes the target of the <code>libfoo.2.dylib</code> symlink that is used by the runtime dynamic linker.
</p>
