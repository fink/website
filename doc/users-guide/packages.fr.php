...
<h1>Guide utilisateur - 3. Installation de paquets</h1>
      ...
      <p>
Maintenant que vous avez ce que l'on pourrait appeler une installation Fink, ce chapitre vous explique comment installer les paquets pour lesquels vous vous êtes donné tout ce mal.
Avant de passer à l'explication proprement dite de l'installation des paquets à partir des distributions source ou binaire, il convient de noter certains points importants qui s'appliquent aux deux types d'installation.
</p>
    
    <h2><a name="bin-dselect">3.1 Installation de paquets binaires avec dselect</a></h2>
      
      <p>
        <code>dselect</code> est un programme qui vous permet de naviguer dans la liste des paquets disponibles et de choisir ceux que vous voulez installer.
Il tourne sous Terminal.app, en mode plein écran, et utilise un système de navigation rudimentaire basé sur les touches du clavier.
Comme tous les autres outils de gestion de paquets, <code>dselect</code> nécessite les privilèges du super-utilisateur ; vous devez donc soit devenir super-utilisateur avant de l'utiliser, soit utiliser sudo :  
</p>
      <pre>sudo dselect</pre>
      <p>
        <b>Note :</b>
        <code>dselect</code> ne cohabite pas très bien avec l'application Terminal de Mac OS X. Vous devez exécuter les commandes suivantes avant de l'utiliser, ou les mettre dans le fichier de démarrage approprié  (<code>.cshrc</code> ou <code>.profile</code>) :</p>
...
Le menu principal vous laisse le choix entre plusieurs actions :
      ...
      <ul>
        <li>
          <p>
            <b>[A]ccess - Accès</b> - permet de configurer la méthode d'accès au réseau.
<b>Il n'est pas nécessaire d'exécuter cette commande</b>, car Fink effectue la configuration à votre place.
En fait, vous devez éviter d'utiliser cette option, car vous risqueriez en le faisant de remplacer la configuration par défaut, qui fonctionne, par une configuration qui ne fonctionne pas.
</p>
        </li>
        <li>
          <p>
            <b>[U]pdate- Mise à jour</b> - permet de télécharger la liste des paquets disponibles sur le site de Fink.
Cette commande n'installe et ne met à jour aucun paquet, elle ne fait que mettre à jour les listes utilisées par le navigateur de paquets. 
Vous devez l'utiliser au moins une fois après avoir installé Fink.
</p>
        </li>
        <li>
          <p>
            <b>[S]elect - Sélection</b> - donne accès à la liste des paquets et permet de sélectionner / désélectionner les paquets que vous désirez.
De plus amples détails seront donnés plus loin.
</p>
        </li>
        <li>
          <p>
            <b>[I]nstall - Installation</b> - c'est là que tout se passe.
Les articles de menus ci-dessus n'affectent que les listes de paquets de dselect et le le statut de la base de données.
Cette commande va télécharger et installer les paquets sélectionnés auparavant.
Elle supprime aussi les paquets désélectionnés dans le navigateur.
</p>
        </li>
        <li>
          <p>
            <b>[C]onfig - Configuration</b> et <b>[R]emove - Suppression</b>.
Ce sont d'anciennes commandes qui datent de la période antérieure à apt.
Vous n'en avez pas besoin, mais elles ne sont pas dangereuses.
</p>
        </li>
        <li>
          <p>
            <b>[Q]uit - Quitter</b> - commande qui dit bien ce qu'elle veut dire.
</p>
        </li>
      </ul>
      <p>
Vous passerez la plupart de votre temps dans le navigateur de paquets de dselect, que vous pouvez activer par le menu "[S]elect".
Avant d'afficher la liste des paquets, dselect affiche un écran d'aide.
Vous pouvez appuyer sur la touche 'k' pour avoir la liste complète des raccourcis clavier ou appuyez sur la barre d'espacement pour obtenir la liste des paquets.
</p>
      <p>
Vous pouvez vous déplacer dans la liste en utilisant les flèches directionnelles haute et basse.
La sélection se fait à l'aide des touches '+' et '-'.
...
Vous pouvez aussi modifier les choix de dselect dans la liste de dépendances (par exemple, choisir un autre paquet pour une dépendance virtuelle), ou appuyez sur 'Majuscules-R' pour retourner à l'état précédent.
Vous quitterez la liste des dépendances et la liste principale des paquets en appuyant sur la touche retour chariot.
Quand vous avez fini vos sélections, quittez la liste principale et utilisez le menu "[I]nstall" pour installer les paquets.
</p>
    
    <h2><a name="bin-apt">3.2 Installation de paquets binaires avec
apt-get</a></h2>
      
      <p>
        <code>dselect</code> ne télécharge pas les paquets lui-même.
        Il utilise apt pour ce faire.
        Si vous préférez utiliser une interface en ligne de commande, vous pouvez accéder à apt directement avec la commande <code>apt-get</code>.
</p>
      <p>
Tout comme avec dselect, vous devez d'abord télécharger la liste des paquets disponibles avec cette commande :
      ...
      <p>
Comme le menu "[U]pdate - Mise à jour" dans dselect, cette commande ne fait que mettre à jour la liste des paquets disponibles, mais ne met pas à jour les paquets eux-mêmes sur votre ordinateur.
Pour installer un paquet, il suffit de donner son nom à apt-get comme dans la commande suivante :
</p>
      <pre>sudo apt-get install lynx</pre>
      <p>
Si apt-get détecte que le paquet requiert certaines dépendances, il vous en affiche la liste et vous demande de confirmer.
Puis il télécharge et installe les paquets choisis.
La suppression de paquets est tout aussi simple :
</p>
...
Pour obtenir la liste des paquets disponibles à partir des sources, utilisez l'outil <code>fink</code> :
      ...
      <pre>fink list</pre>
      <p>
La première colonne de la liste affiche le statut d'installation (vide si le paquet n'est pas installé, <code>i</code> s'il est installé et <code>(i)</code> s'il existe une version plus récente que la version installée. La seconde colonne donne le nom du paquet, la troisième sa version et la dernière une brève description.
Vous pouvez obtenir une description plus détaillée d'un paquet particulier avec la commande 
"describe" ( ou "info") :
</p>
      <pre>fink describe xmms</pre>
      <p>
Si vous voulez installer un paquet, utilisez la commande 
"install" :
</p>
      <pre>fink install wget-ssl</pre>
      <p>
La commande <code>fink</code> vérifie tout d'abord que toutes les "dépendances" requises sont présentes sur votre système. Si ce n'est pas le cas, elle vous demandera de confirmer l'installation de celles qui sont manquantes.
...
Tout ceci peut prendre un certain temps.
...
<li><b>current-10.3 unstable :</b>  C'est la version instable la plus récente qui puisse être installée à partir du source sur OS 10.3.  Activez l'arborescence instable comme indiqué ci-dessus.</li>
    ...
    <h2><a name="x11">3.7 Implémentation de X11</a></h2>
      ...
      <p>
Comme il existe plusieurs implémentations possibles de X11 sur Mac OS X (XFree86, Tenon Xtools, eXodus) et plusieurs façons de les installer (manuellement ou via Fink), vous avez le choix entre plusieurs paquets - un par type d'implémentation/installation.
Fink ne sait pas toujours très bien déterminer si X11 est installé sur votre système et sous quelle forme, il est donc très important de choisir le bon paquet et de l'installer avant de commencer à installer des applications X11.
Voici la liste des paquets disponibles et des méthodes d'installation de X11 :
</p>
      <ul>
        <li>
          <p>
            <b>xfree86-base :</b>
(uniquement pour 10.1 ou 10.2) Ce paquet correspond à une installation réelle.
Il installe le paquet Fink XFree86 4.2.1.1.
Pour assurer un maximum de flexibilité, ce paquet ne contient pas le serveur XDarwin.
Pour disposer du serveur, vous devez installer le paquet xfree86-rootless.
</p>
        </li>
        <li>
          <p>
            <b>xfree86 :</b>
C'est un paquet complet (le serveur d'affichage est inclus) qui installe XFree86 4.3.0 (uniquement pour 10.2), ou 4.3.99 (uniquement pour 10.3).
Cette version est plus performante que la version 4.2.1.1, mais n'a pas été testée autant.
</p>
        </li>
        <li>
          <p>
system-xfree86 :
Ce paquet est généré automatiquement (avec Fink 0.6.2 ou une version ultérieure) quand vous installez manuellement XFree86, soit à partir du source, soit à partir de la distribution officielle (ou non officielle) binaire ; ou quand vous installez X11 d'Apple.
Il tient lieu alors de  paquet fantôme pour les dépendances.
</p>
        </li>
        <li>
          <p>
system-xtools :
InstaIlez ce paquet si vous avez installé le progiciel Xtools de Tenon.
Tout comme system-xfree86, ce paquet ne fait que vérifier si l'installation est correcte et ne modifie pas les fichiers.
</p>
        </li>
      </ul>
      <p>
Pour de plus amples informations sur l'installation et l'utilisation de X11, voir le document 
