...
<h1>Guide utilisateur - 6. Utilisation de l'outil fink en ligne de commande</h1>
    ...
    <h2><a name="options">6.2 Options globales</a></h2>
      ...
      <p>
Ce sont des options qui s'appliquent à toutes les commandes fink. Pour obtenir la liste des options, exécutez <code>fink --help</code> :
      </p>
      <pre>
...
-b, --use-binary-dist - télécharge les paquets pré-compilés de la distribution binaire s'ils sont disponibles
      ...
      <p>
La plupart de ces options ne nécessite pas d'explication supplémentaire. Elles peuvent être configurées dans le <a href="conf.php?phpLang=fr">fichier de configuration de Fink</a> (fink.conf) si vous souhaitez les rendre permanentes et non pas restreindre leur effet à une seule invocation de <code>fink</code>.
      </p>
      <p>
L'option <code>--use-binary-dist</code> force <code>fink</code> à télécharger les paquets binaires à partir de la distribution binaire, s'ils sont disponibles et qu'ils ne sont pas déjà sur votre système. Disponible à partir de la version 0.23.0 de <code>fink</code>.
      </p>
...
<p>Ici fink lit les fichiers d'information des paquets, donne le nombre de fichiers lus, signale que le paquet lesstif sera téléchargé si vous donnez votre accord.</p>      
      <p>
Si vous ajoutez l'<a href="#options">option --use-binary-dist</a>, <code>fink</code> télécharge les paquets binaires, s'ils sont disponibles, au lieu de les compiler. Cela permet de gagner du temps.
      </p>
