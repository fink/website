...
Ce chapitre explique les procédures utilisées pour mettre à niveau votre installation de Fink à la toute dernière version.
    ...
    <h2><a name="bin">4.1 Mise à niveau à partir de paquets binaires</a></h2>
      ...
      <p>
Si vous utilisez exclusivement la distribution binaire, vous n'avez pas à suivre une procédure de mise à niveau particulière.
Il vous suffit d'utiliser l'outil que vous préférez pour récupérer la liste des paquets les plus récents et lui laisser mettre à jour tous les paquets.
</p>
      <p>
Avec dselect, il suffit d'appuyer sur la touche "[U] - Mise à jour", puis sur  "[I] - Installation".
Vous pouvez, bien sûr, utiliser "[S] - Sélectionner" entre les deux commandes pour vérifier les choix faits et rechercher de nouveaux paquets.
</p>
      <p>
Avec apt, lancez <code>apt-get update</code> pour récupérer la liste des paquets les plus récents, puis <code>apt-get upgrade</code> pour mettre à jour tous les paquets qui peuvent l'être.
</p>
<p>Avec Fink Commander, sélectionnez Binary-&gt;Update descriptions pour mettre à jour la liste des paquets, puis  Binary-&gt;Dist-Upgrade packages pour mettre à jour les paquets eux-mêmes.</p>
      <p>
Pour de plus amples informations, en particulier sur la mise à niveau des versions de Fink antérieure à la version 0.3.0, voir la 
<a href="http://fink.sourceforge.net/download/upgrade.php">Matrice de mise à niveau</a>.
</p>
...
Dans un deuxième temps, ces descriptions de paquets sont utilisées pour recompiler les paquets ; le code source est téléchargé, si besoin est.
      ...
      <p>
Si vous avez Fink 0.2.5 ou une version ultérieure, vous pouvez réaliser la première partie de la procédure en lançant <code>fink selfupdate</code>.
Cette commande vérifie sur le site web de Fink s'il existe une nouvelle version, la télécharge automatiquement et installe les descriptions de paquets.
Avec les versions récentes de la commande <code>fink</code>, vous pouvez récupérer les descriptions de paquets via CVS ou rsync.
CVS est un serveur de contrôle de version sur lequel sont stockées et gérées les descriptions de paquets.
L'utilisation de CVS présente l'avantage qu'il est continuellement mis à jour, mais il faut savoir qu'il n'existe qu'un seul serveur CVS et qu'il peut devenir non fiable lorsqu'il subit un trafic intense. C'est pourquoi nous recommandons à l'utilisateur moyen de se servir de rsync. Il existe différents miroirs pour rsync et le seul désavantage de cette méthode est qu'il faut compter environ une heure pour que les descriptions de paquets parviennent sur les miroirs rsync après leur mise à jour sur CVS.  
      ...
      <p>(si vous avez des problèmes lors d'une mise à niveau à partir d'une installation source, consultez 
<a href="http://fink.sourceforge.net/download/fix-upgrade.php">ces instructions spéciales</a>.)</p>
      <p>
Si vous avez une version de Fink antérieure à 0.2.5, vous devez télécharger les descriptions de paquets vous-même.
Allez dans la <a href="http://sourceforge.net/project/showfiles.php?group_id=17203">zone de téléchargement</a> et cherchez l'archive tar la plus récente nommée packages-0.x.x.tar.gz dans le module "distribution".
Téléchargez-la et installez-la comme suit :
</p>
      <pre>tar -xzf packages-0.x.x.tar.gz
cd packages-0.x.x
./inject.pl</pre>
      <p>
Après la mise à jour des descriptions de paquets (quelle que soit la méthode employée), vous devrez mettre à jour tous les paquets à l'aide de la commande <code>fink
update-all</code>.
</p>
