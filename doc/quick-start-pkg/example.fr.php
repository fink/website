...
<p>Nous pouvons maintenant sauvegarder ceci sous le nom <code>maxwell.info</code> dans le répertoire <code>/opt/sw/fink/dists/local/main/finkinfo/</code>. Ceci fait, voyons ce que cela donne avec <code>fink validate</code>.</p>
<pre>
...
<p>Hum ! Ça ne marche pas très bien. Lisons le README (situé dans <code>/opt/sw/src/maxwell-0.5.1-1/maxwell-0.5.1/README</code>) et voyons ce qu'il dit...</p>
<pre>
To build type 'make'.
...
<p>Nous devons utiliser <code>prefix=%i</code> car <code>fink</code> construit le fichier binaire à partir des fichiers se trouvant dans <code>%i</code>. Ces fichiers seront ensuite installés dans <code>%p</code> (qui correspond par défaut à <code>/opt/sw</code>) quand on exécutera <code>fink install maxwell</code>. Pour de plus amples informations sur <code>%p</code> et <code>%i</code>, consultez le <a href="/doc/packaging/format.php#percent">Guide de construction des paquets</a>.</p>
...
<p>Fink semble avoir tout installé au bon endroit : <code>/opt/sw/src/root-maxwell-0.5.1-1</code> à partir de l'emplacement où le paquet binaire <code>maxwell_0.5.1-1_darwin-powerpc.deb</code> a été construit.</p>
...
<p>Heu ! Quelque chose ne va pas. Consultons encore le <a href="/doc/packaging/fslayout.php#fhs">Guide de construction des paquets</a>. On y voit que les pages man doivent être installées dans <code>/opt/sw/share/man</code> et les fichiers <code>README</code> dans <code>/opt/sw/share/doc/%n</code>. Si nous ouvrons le <code>Makefile</code> de maxwell, nous voyons que nous pouvons définir les répertoires mandir et datadir :</p>
<pre>
prefix = /usr/local
mandir = ${prefix}/man
man8dir = ${mandir}/man8
