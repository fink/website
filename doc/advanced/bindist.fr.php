...
$cvs_date = 'Date: 2005/03/02 05:22:33';
...
<p>Cette section décrit une méthode d'utilisation d'un serveur central de compilation ( serveur "maître") pour distribuer des paquets binaires à tous les clients des stations de travail. Elle s'applique aux stations de travail utilisant plusieurs installation de Fink.</p>
<p><b>Note :</b> dans les instructions suivantes, on a supposé que toutes les machines clients ont une version de fink &gt;= 0.24.0. Voir plus loin la section <a href="#remarks">remarques</a> si les machines ont une version de fink antérieure.</p>
<p>La méthode se compose des étapes suivantes sur le <a href="#master">serveur "maître"</a> et sur les <a href="#client">machines clients</a>:</p>
...
<p>Si vous utiliser le paquet apache2 de Fink, vous devrez modifier les chemins ci-dessus.</p>

<h2><a name="client">1.3 Étapes sur les machines clients</a></h2>
...
à la fin du processus de mise à jour, si le niveau de verbosité est &gt;= 1.</li>
</ol>
...
<ul>
<li>Le serveur "maître" doit utiliser une version de X11 identique à la plus ancienne version installée sur l'ensemble des clients. Par exemple, si toutes les machines clientes utilisent X11 d'Apple, le serveur "mâitre" doit l'utiliser lui aussi.</li>
<li>Vous pouvez supprimer les paquets qui ne sont que des dépendances de compilation (c'est-à-dire qui ne sont pas nécessaire à l'exécution d'autres paquets). Le paquet <code>debfoster</code> permet de le faire facilement. Faîtes toutefois attention à ne pas supprimer de paquets essentiels, tel <code>apt</code>.</li>
<li>Si vous utilisez une version de fink inférieure à la version 0.24.0 sur une machine client, vous devez exécuter <code>sudo apt-get update</code> au lieu de <code>fink selfupdate</code>. Vous devez alors installer les paquets binaires via <code>sudo apt-get install &lt;package&gt;</code>. </li>
</ul>
