...
除非有特别的理由，否则静态和动态连接库应该直接保存在 <code>/opt/sw/lib</code>。
这里通常还保存那些不应该由用户直接运行的可执行程序(否则的话，应该放在 libexec)。
</p>
<p>
软件包可以创建子目录来存放私有数据或可加载模块。
请确定使用那些对保持兼容性有利的目录名。
在目录名或目录层次中使用主版本号是个好办法，例如：<code>/opt/sw/lib/perl5</code>
或 <code>/opt/sw/lib/apache/1.3</code>。
...
<p>
This directory is for 64-bit libraries, with <code>/opt/sw/lib/ppc64</code>
...
<p>
除了上面提到的以外，在 <code>/opt/sw</code> 中不应该有其它的目录。
特别地，不应该使用下面这些目录：
