...
<h1>パッケージ作成 - 4. ファイルシステムのレイアウト</h1>
		...
		<h2><a name="fhs">4.1 ファイルシステム構造標準 (Filesystem Hierarchy Standard)</a></h2>
			...
			<p>
				...
				以降の例ではデフォルトの「プリフィクス」 <code>/opt/sw</code> を使います．
			</p>
		...
		<h2><a name="dirs">4.2 ディレクトリ</a></h2>
			...
			<table border="0" cellpadding="0" cellspacing="10"><tr valign="bottom"><th align="left">Field</th><th align="left">Value</th></tr><tr valign="top"><td>
						...
						<code>/opt/sw/lib</code>
					</td><td>
						<p>
							アーキテクチャ依存のデータファイルやライブラリ用．
							静的および共有ライブラリは，避ける理由が特にない限り <code>/opt/sw/lib</code> 直下に置きます．
						...
						<p>
							...
							(<code>/opt/sw/lib/perl5</code> や <code>/opt/sw/lib/apache/1.3</code> など)．
...
powerpc アーチテクチャーでは <code>/opt/sw/lib/ppc64</code> が，
i386 アーチテクチャーでは <code>/opt/sw/lib/x86_64</code> が用いられます．
'fat' としてビルドされたライブラリは， <code>/opt/sw/lib</code> に保存されます．
</p>
</td></tr><tr valign="top"><td>
						<code>/opt/sw/share</code>
					</td><td>
						<p>
							アーキテクチャに依存しないデータファイル用で， <code>/opt/sw/lib</code> と同じルールが当てはまります．
							よく使われるサブディレクトリについては後述します．
						</p>
					</td></tr><tr valign="top"><td>
						<code>/opt/sw/share/man</code>
					</td><td>
						<p>
							man ページ用．
							この中は man のセクションに従って分類されます．
							<code>/opt/sw/bin</code> と <code>/opt/sw/sbin</code> の中の全てのプログラムには，
							対応した man ページがここになければいけません．
						</p>
					</td></tr><tr valign="top"><td>
						<code>/opt/sw/share/info</code>
					</td><td>
						<p>
						...
						<code>/opt/sw/share/locale</code>
					</td><td>
						<p>
							国際化で使うメッセージカタログ用．
						</p>
					</td></tr><tr valign="top"><td>
						<code>/opt/sw/var</code>
					</td><td>
						<p>
...
<p>このディレクトリには，OS X 型のアプリケーションが使用する，OS X 型のフレームワークを保存する．</p>
		...
		<h2><a name="avoid">4.3 避けるべきこと</a></h2>
			...
			<p>
				<code>/opt/sw</code> 下には，上述のもの以外ディレクトリを作ってはいけません．
				特に以下のディレクトリを作らないこと:
				<code>/opt/sw/man</code>, <code>/opt/sw/info</code>, <code>/opt/sw/doc</code>,
				<code>/opt/sw/libexec</code>, <code>/opt/sw/lib/locale</code>
			</p>
