...
<div class="answer"><p><b>A:</b> Typically these errors take the form:</p><p><tt><nobr>files list file for package <b>packagename</b> contains empty filename</nobr></tt></p><p>or</p><p><tt><nobr>files list file for package <b>packagename</b> is missing final newline</nobr></tt></p><p>This can be fixed, with a little work.  If you have the .deb file for the offending package currently available on your system, then check its integrity by running</p><p><tt><nobr>dpkg --contents <b>full-path-to-debfile</b></nobr></tt></p><p>e.g.</p><pre>dpkg --contents /sw/fink/10.2/unstable/main/binary-darwin-powerpc/gnome/libgnomeui2-dev_2.0.6-2_darwin-powerpc.deb</pre><p>If you get back a listing of directories and files, then your .deb is OK.  If the output is something other than directories and files, or if you don't have the deb file, you can still proceed because the error doesn't interfere with builds.  Just use <tt><nobr>fink rebuild <b>packagename</b></nobr></tt>, to build the .deb--it won't install yet, though.</p><p>Once you have a valid .deb file, then you can reconstitute the file using the following command:</p><p><tt><nobr>dpkg -c <b>full-path-to-debfile</b> | awk '{if ($6 == &quot;./&quot;) { print &quot;/.&quot;; } else if (substr($6, length($6), 1) == &quot;/&quot;) {print substr($6, 2, length($6) - 2); } else { print substr($6, 2, length($6) - 1);}}' &gt; /sw/var/lib/dpkg/info/packagename.list</nobr></tt></p><p>e.g.</p><pre>dpkg-deb -c /sw/fink/10.2/unstable/main/binary-darwin-powerpc/gnome/libgnomeui2-dev_2.0.6-2_darwin-powerpc.deb | awk '{if ($6 == &quot;./&quot;) { print &quot;/.&quot;; } else if (substr($6, length($6), 1) == &quot;/&quot;) {print substr($6, 2, length($6) - 2); } else { print substr($6, 2, length($6) - 1);}}' &gt; /sw/var/lib/dpkg/info/libgnomeui2-dev.list</pre><p>What this does is to print out the contents of the .deb file, remove everything but the filenames, and write these to the .list file.</p></div></a>
<p align="right">
