...
<h1>F.A.Q. - 9 Package Usage Problems - Specific Packages</h1><a name="xmms-quiet"><div class="question"><p><b>Q9.1: I get no sound from XMMS</b></p></div><div class="answer"><p><b>A:</b> Make sure you have the "eSound Output Plugin" selected in the XMMS
        ...
        find your sound card try this:</p><ul><li>Make sure you haven't muted sound output in Mac OS X.</li><li>Run <code>esdcat /usr/libexec/config.guess</code> (or any other
          ...
          up manually with <code>esd &amp;</code> and watch the messages.</li><li>If it still doesn't work, check the permissions on
          ...
          normal user, not as root).</li></ul><p>Note that esd is designed to be run by a normal user, not by root.
        ...
        We don't have an analysis or a fix yet.</p></div></a><a name="nedit-window-locks"><div class="question"><p><b>Q9.2: If I am editing a file in nedit, when I open another file its
        window pops up but is unresponsive.</b></p></div><div class="answer"><p><b>A:</b> This is a known problem that occurs with recent versions of
        ...
        <code>lesstif</code>.</p></div></a><a name="xdarwin-start"><div class="question"><p><b>Q9.3: Help! When I start XDarwin, it immediately quits!</b></p></div><div class="answer"><p><b>A:</b> Don't Panic. The Running X11 document now has an extensive <a href="http://fink.sourceforge.net/doc/x11/trouble.php#immediate-quit">troubleshooting
        section</a> for this common problem.</p></div></a><a name="no-server"><div class="question"><p><b>Q9.4: When I try to start XDarwin I get the message "xinit: No such file
        or directory (errno 2): no server "/usr/X11R6/bin/X" in PATH".</b></p></div><div class="answer"><p><b>A:</b> First, make sure you are sourcing init.sh in your X startup
        ...
        <code>/usr/bin/X11R6</code> in your PATH.</p></div></a><a name="xterm-error"><div class="question"><p><b>Q9.5: xterm fails with "dyld: xterm Undefined symbols: xterm undefined
        reference to _tgetent expected to be defined in
        /usr/lib/libSystem.B.dylib".</b></p></div><div class="answer"><p><b>A:</b> This is caused by using a 10.1 version of XFree86 on 10.2. You must
        ...
        web site</a>.</p></div></a><a name="libXmuu"><div class="question"><p><b>Q9.6: When I try to start XFree86 I get one of the following errors:
        "dyld: xinit can't open library: /usr/X11R6/lib/libXmuu.1.dylib" or
        "dyld: xinit can't open library: /usr/X11R6/lib/libXext.6.dylib"</b></p></div><div class="answer"><p><b>A:</b> You are missing a file that is supposed to be installed by
        ...
        --reinstall xfree86-base-shlibs</code> for binaries.</p></div></a><a name="apple-x-bugs"><div class="question"><p><b>Q9.7: I had Fink's XFree86 installed, and I've replaced it with Apple's
        X11, and now everything's crashing!</b></p></div><div class="answer"><p><b>A:</b> First of all, if you previously had the "threaded" versions of
        ...
        Apple bug reporter</a>.</p></div></a><a name="apple-x-delete"><div class="question"><p><b>Q9.8: I want the delete key in Apple's X11.app to behave like that in
        XDarwin.</b></p></div><div class="answer"><p><b>A:</b> Some users have reported that the behavior of the
...
xmodmap $HOME/.Xmodmap</pre><p></p></div></a><a name="gnome-two"><div class="question"><p><b>Q9.9: I upgraded from GNOME 1.x to GNOME 2.x and now
        <code>gnome-session</code> won't open a window manager.</b></p></div><div class="answer"><p><b>A:</b> While under GNOME 1.x <code>gnome-session</code> invokes the
...
exec metacity &amp; exec gnome-session</pre><p>Note:  this is no longer true for <b>GNOME 2.4</b>.  Running <code>gnome-session</code> invokes a window manager.</p></div></a><a name="apple-x11-no-windowbar"><div class="question"><p><b>Q9.10: I upgraded to Apple's X11 in Panther and now my window title bars
        are missing.</b></p></div><div class="answer"><p><b>A:</b> You didn't upgrade X11 to version "X11 1.0 - XFree86 4.3.0"
        included with Panther. You can install X11 from X11.pkg on Disk 3.</p></div></a><a name="apple-x11-wants-xfree86"><div class="question"><p><b>Q9.11: I installed Apple's X11 in Panther but Fink keeps asking to install
        xfree86.</b></p></div><div class="answer"><p><b>A:</b> There are two possibilities to consider.</p><ul><li><b>You are installing from binaries:</b> If you are running a
...
sudo apt-get install fink</pre></li><li><b>You are installing from source: </b>You should first update
          ...
          <code>Packages</code> folder.</li></ul><p>For either case, you can check your installation by running</p><pre>fink-virtual-pkgs</pre><p>and checking to see that the
...
/Library/Receipts/X11User.pkg; fink selfupdate; fink index</pre><p>Then, reinstall X11 from the third Panther CD (and the X11SDK from the
        ...
        download and run to get the same information.</p><ul><li>Get it here: <a href="http://perso.wanadoo.fr/costabel/fink-x11-debug">http://perso.wanadoo.fr/costabel/fink-x11-debug</a></li><li>Save it wherever you like.</li><li>Run it in a terminal window via <pre>perl fink-x11-debug</pre></li></ul></div></a><a name="apple-x11-beta-wants-xfree86"><div class="question"><p><b>Q9.12: I installed Apple's X11 with the 10.2-gcc3.3 version of Fink but
        Fink keeps asking to install xfree86.</b></p></div><div class="answer"><p><b>A:</b> There are two possibilities to consider.</p><ul><li><b>You are installing from binaries:</b> If you are running a
          version of the <code>fink</code> package that is earlier than 0.17.0
          (such as the version that comes with the Fink-0.6.2 installer), then
          updating fink may solve your problem immediately, e.g.
          via<pre>sudo apt-get update 
sudo apt-get install fink</pre></li><li><b>You are installing from source: </b>You should first update
          ...
          Apple's X11.</li></ul><p>For either case, you can check your installation by running</p><pre>fink-virtual-pkgs</pre><p>and checking to see that the
