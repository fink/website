...
$metatags = "<link rel=\"contents\" href=\"index.php?phpLang=zh\" title=\"常见疑问（F.A.Q.） Contents\">\n\t<link rel=\"next\" href=\"comp-general.php?phpLang=zh\" title=\"一般性编译问题\">\n\t<link rel=\"prev\" href=\"upgrade-fink.php?phpLang=zh\" title=\"升级 Fink （解决特定版本的问题）\" />";
...
另外，在网站上也有一个<a href="http://fink.sourceforge.net/pdb/">软件包数据库</a>。
    ...
    <a name="proxy">
      ...
      <div class="answer"><p><b>A:</b> <code>fink</code> 命令可以设定代理服务器，并把它传递给 <code>wget</code>/<code>curl</code>。
...
一个 apt 软件包修正版（提供了配制脚本作为 dselect 的插件）可以通过 CVS 获得。
    ...
    <a name="cvs-busy">
      <div class="question"><p><b>Q5.12: 当我试图运行 <q>fink selfupdate</q> 或 "fink selfupdate-cvs" 时，我碰到了 "<code>Updating using CVS failed. Check the error messages above.</code>"这个错误信息。</b></p></div>
...
fink selfupdate-cvs</pre></div>
    ...
    <a name="kernel-panics">
      <div class="question"><p><b>Q5.13: 当我使用 Fink 的时候，碰到整个机器没有反应/核心恐慌/死机。救命！</b></p></div>
      <div class="answer"><p><b>A:</b>  最近在
<a href="http://sourceforge.net/mailarchive/forum.php?forum=fink-users">fink 用户邮件列表</a> 中有不少关于这个问题的反映（包括核心恐慌和不断旋转的彩轮），这通常在安装有防病毒软件的机器上编译软件包时发生。也许在使用 Fink 之前你应该关闭所有防病毒软件。
    ...
    <a name="not-found">
      <div class="question"><p><b>Q5.14: 我试图安装一个软件包，但 Fink 不能下载它。下载的网站显示一个比 Fink 里面更新的版本号。我该怎么办？</b></p></div>
      <div class="answer"><p><b>A:</b> 当新版本发布的时候，源程序包会被从上游网站移走。</p><p>首先你应该做的是运行 <code>fink selfupdate</code>。
...
<a href="http://sourceforge.net/mailarchive/forum.php?forum=fink-users">fink 用户邮件列表</a> 里面发信询问是否有人会有旧的源代码压缩档可以给你。</p><p>一旦你找到合适的源程序压缩档，手工下载它，然后把它移到 Fink 的源程序文件目录里面（对默认安装，可以使用 "<code>sudo mv <b>package-source.tar.gz</b> /sw/src/</code>"命令）。然后和平常一样用 '<code>fink install <b>软件包名</b>
  ...
  &gt; /sw/var/lib/dpkg/info/libgnomeui2-dev.list</pre><p>这里做的事情是抽取 .deb 文件的内容，删掉除文件名以外的所有信息，并把文件名信息写回到 .list 文件中。</p></div>
    ...
    <a name="error-nineteen">
      <div class="question"><p><b>Q5.19: 当我使用 Fink 二进制安装包的时候，我碰到一个很大的 "19" 在窗口中，而不能安装任何东西。</b></p></div>
      <div class="answer"><p><b>A:</b> 显示数字 19 是因为你的 OS X 系统被本地化为别的语言。（这是苹果安装程序的一个问题，它这时没有显示英语的错误信息）。</p><p>对应于错误 19 的英语错误信息是：</p><p>"/sw 目录已存在，请参阅自述文件已获取升级指南，或关于如何安装 Fink 到一个单独的宗卷。"</p><p>如果你以前使用过 Fink，而又没有删除 <code>/sw</code> 目录，你会碰到这个错误信息。如果你以前没有安装过 Fink，最大的可能是你作为 .Mac 用户免费安装了 Virex 程序。
正如 Fink 的网页所解释的一样，Virex 与 Fink 不兼容（因为 Virex 错误的设置方式）。</p></div>
    </a>
    <a name="dselect-garbage">
      <div class="question"><p><b>Q5.20: 当我在 <code>dselect</code> 中选择软件包时，屏幕显示一堆乱七八糟的东西。怎么办？</b></p></div>
      <div class="answer"><p><b>A:</b> 在 <code>dselect</code> 和 终端程序（<code>Terminal.app</code>）中间存在一些问题。暂时解决方法是在运行 <code>dselect</code> 前输入下面的命令。</p><p>对 tcsh 用户：</p><pre>setenv TERM xterm-color</pre><p>对 bash 用户：</p><pre>export TERM=xterm-color</pre><p>你可以把它放到你的启动文件（比如 <code>.cshrc</code> 或 <code>.profile</code>），这样它就总是会自动运行。</p></div>
    </a>
    <a name="perl-undefined-symbol">
      <div class="question"><p><b>Q5.21: 为什么我运行 Fink 命令时会有一堆的 "dyld: perl undefined symbols" 错误？</b></p></div>
      <div class="answer"><p><b>A:</b> 如果你看到象下面的错误信息：</p><pre>dyld: perl Undefined symbols:
_Perl_safefree
_Perl_safemalloc
_Perl_saferealloc
_Perl_sv_2pv
...
fink selfupdate-cvs</pre></div>
    ...
    <a name="cant-upgrade">
      <div class="question"><p><b>Q5.22: 我看不到要升级的 Fink 版本。</b></p></div>
      <div class="answer"><p><b>A:</b> 在这种情况下，参考<a href="http://fink.sourceforge.net/download/fix-upgrade.php">特别指引</a>。</p></div>
    </a>
    <a name="spaces-in-directory">
      <div class="question"><p><b>Q5.23: 我可以把 Fink 放到一个名字里面有空格的目录或宗卷里面吗？</b></p></div>
      <div class="answer"><p><b>A:</b> 我们不推荐把你的 Fink 目录树放到名字中有空格的目录里面。完全不值得冒这个险。</p></div>
    </a>
    <a name="packages-gz">
      <div class="question"><p><b>Q5.24: 当我进行二进制升级的时候，有很多 "File not found" 错误。</b></p></div>
...
update available list script returned error exit status 1.
    ...
    <a name="wrong-tree">
      <div class="question"><p><b>Q5.25: 我更改了我的错误系统或开发工具，但 Fink 没有认出这些改动。</b></p></div>
    ...
    <a name="seg-fault">
      <div class="question"><p><b>Q5.26: 当我运行<code> fileutils </code>中的 <code>gzip</code> 或 <code>dpkg-deb</code> 程序时出现错误！救命！</b></p></div>
...
set up for Fink already."，然后它就退出登录了。</b></p></div>
  ...
  <p align="right">
Next: <a href="comp-general.php?phpLang=zh">6 一般性编译问题</a></p><? include_once "../footer.inc"; ?>
