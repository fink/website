...
如果你按照安装指南进行安装，并使用 <code>/opt/sw/bin/init.csh</code>（或 <code>/opt/sw/bin/init.sh</code>），那么 <code>apt-get</code> 和 <code>dselect</code> 也可以使用这些设置。请确认你已经把协议名放在代理服务器域名之前，例如：</p><pre>ftp://proxy.yoursite.somewhere</pre><p>如果你仍然碰到问题，那么打开系统预置，选择网络设置面板，确认 "使用被动 FTP 模式 (PASV)" 选项已被选中。</p></div>
    </a>
    <a name="firewalled-cvs">
      ...
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> <b>cvs-proxy</b> 这个软件包可以通过 HTTP 代理穿透防火墙。</p><ul>
          <li>
            <p>首先下载 <a href="http://fink.cvs.sourceforge.net/fink/dists/10.2/unstable/main/finkinfo/devel/">cvs-proxy</a> 文件（一个 .info 文件和一个 .patch 文件）并把它放到你的本地代码树中（即 /opt/sw/fink/dists/local/main/finkinfo/）。</p>
          </li>
          <li>
...
通常的猜测这应该可行，但也许会有些陷阱在某些地方。
    ...
    <a name="removing">
      ...
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 
Fink 的所有文件几乎都安装在 /opt/sw （或你选择安装的地方）。因此，如果你想删除 Fink，输入下面的命令：
</p><pre>sudo rm -rf /opt/sw</pre><p>
...
/opt/sw/bin/init.csh</code>" 这一行；或从 <code>.bashrc</code> 文件中删除 "<code>source /opt/sw/bin/init.sh</code>" 这一行。</p></div>
    </a>
    <a name="bindist">
...
fink scanpackages
    ...
    <a name="exec-init-csh">
      ...
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> init.csh 和 init.sh 并不能象普通命令一样运行。这些文件会设置象 PATH 和 MANPATH 这样的环境变量到你的 shell 程序中。要对 shell 一直起作用的话，对于 csh/tcsh，它需要由一个 <code>source</code> 命令来运行；或对于 bash/zsh，使用"<code> . </code>"命令，象这样：</p><p>对 csh/tcsh：</p><pre>source /opt/sw/bin/init.csh</pre><p>或对 bash/zsh：</p><pre>. /opt/sw/bin/init.sh</pre></div>
    </a>
...
<code>/opt/sw/etc/apt/sources.list</code> 文件。删除包含 debian.org 的行，并换成：
...
</pre><p>这时你需要重置你的 CVS 目录。使用命令：</p><pre>sudo find /opt/sw/fink -type d -name 'CVS' -exec rm -rf {} \;
fink selfupdate-cvs</pre><p>如果你看不到上面这些错误信息，那么很可能是你自己修改过 /opt/sw/fink/dists 树里面的文件，而现在维护者又改动了它。查看以前的 selfupdate-cvs 输出，寻找以 "C" 开始的行，象这样：
</p><pre>C 10.2/unstable/main/finkinfo/libs/db31-3.1.17-6.info
...
(other info and patch files)
...
### execution of su failed, exit code 1
Failed: Updating using CVS failed. Check the error messages above.</pre><p>"C" 指 CVS 在更新最新版本的时候出现冲突。</p><p>解决办法是删除在 selfupdate-cvs 命令的输出中 "C" 开头的错误信息所涉及的文件，然后重新试一次。</p><pre>sudo rm /opt/sw/fink/10.2/unstable/main/finkinfo/libs/db31-3.1.17-6.info
...
<a href="http://sourceforge.net/mailarchive/forum.php?forum=fink-users">fink 用户邮件列表</a> 里面发信询问是否有人会有旧的源代码压缩档可以给你。</p><p>一旦你找到合适的源程序压缩档，手工下载它，然后把它移到 Fink 的源程序文件目录里面（对默认安装，可以使用 "<code>sudo mv <b>package-source.tar.gz</b> /opt/sw/src/</code>"命令）。然后和平常一样用 '<code>fink install <b>软件包名</b>
    ...
    <a name="invisible-sw">
      <div class="question"><p><b><?php echo FINK_Q ; ?>5.17: 我希望在 Finder 里面隐藏 /opt/sw 而避免用户破坏 Fink 的设置。</b></p></div>
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 如果你安装了开发工具包，你可以做到这一点。你可以运行下面的命令：</p><pre>sudo /Developer/Tools/SetFile -a V /opt/sw</pre><p>这会使得 /opt/sw 象其它标准系统文件夹一样（比如 /usr），在 Finder 中不可见。如果你没有开发工具包，也有其它第三方程序可以让你修改文件属性-你需要把 /opt/sw 设为隐藏。</p></div>
    </a>
    <a name="install-info-bad">
      <div class="question"><p><b><?php echo FINK_Q ; ?>5.18: 我不能安装任何软件，因为我碰到下面的错误信息："install-info: unrecognized option `--infodir=/opt/sw/share/info'"。</b></p></div>
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 这通常是由于你的 PATH 环境变量的原因。在终端窗口输入：</p><pre>printenv PATH</pre><p>如果输出里面没有 <code>/opt/sw/sbin</code>，那么你需要按照用户指南中的<a href="/doc/users-guide/install.php#setup">方法</a>来设置你的环境变量。如果有 <code>/opt/sw/sbin</code>，但有其它目录在它前面（比如 <code>/usr/local/bin</code>），那么你要么需要重新安排你 PATH 里面的顺序，使得 <code>/opt/sw/sbin</code> 排在前面。或者如果你的确需要把其它目录放在 <code>/opt/sw/sbin</code> 之前，而且这个放在前面的目录包括另一个 install-info 目录，这时也许你需要在使用 Fink 的时候临时重命名这个 <code>install-info</code> 子目录。</p></div>
    </a>
  ...
  &gt; /opt/sw/var/lib/dpkg/info/<b>软件包名</b>.list</pre><p>例如：</p><pre>dpkg -c /opt/sw/fink/debs/libgnomeui2-dev_2.0.6-2_darwin-powerpc.deb
  | awk '{if ($6 == "./") { print "/."; } else if (substr($6, length($6), 1) == "/")
   {print substr($6, 2, length($6) - 2); } else { print substr($6, 2, length($6) - 1);}}'
  &gt; /opt/sw/var/lib/dpkg/info/libgnomeui2-dev.list</pre><p>这里做的事情是抽取 .deb 文件的内容，删掉除文件名以外的所有信息，并把文件名信息写回到 .list 文件中。</p></div>
    </a>
...
powerpc_Packages) - stat (2 No such file or directory)</pre><p>那么你需要做的是运行 <code>fink scanpackages</code>。这会生成那些找不到的文件。</p></div>
    ...
    <a name="wrong-tree">
      ...
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 在改动 Fink 的安装环境（源代码或二进制安装都是它的子集），需要告诉 Fink 发生了这些变动。要这样做，我们要运行一个通常是在首次安装的时候才运行的脚本：</p><pre>/opt/sw/lib/fink/postinstall.pl</pre><p>运行完这个脚本以后，Fink 应该能够适应改动的结果。</p></div>
    </a>
    ...
    <a name="seg-fault">
      ...
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 出错的形式：</p><pre>gzip -dc /opt/sw/src/dpkg-1.10.9.tar.gz | /opt/sw/bin/tar -xf -
### execution of gzip failed, exit code 139</pre><p>或</p><pre>gzip -dc /opt/sw/src/aquaterm-0.3.0a.tar.gz | /opt/sw/bin/tar -xf -
...
Failed: can't create package base-files_1.9.0-1_darwin-powerpc.deb</pre><p>或在运行<code> fileutils</code> 中的工具时出现 segmentation faults 错误。比如：<code>ls</code> 或 <code>mv</code>，这很可能时因为某个库的预绑定错误，这可以通过运行下面命令来修正：</p><pre>sudo /opt/sw/var/lib/fink/prebound/update-package-prebinding.pl -f</pre></div>
    </a>
...
set up for Fink already."，然后它就退出登录了。</b></p></div>
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> 发生这个现象的原因是 OS X 的终端程序被告知每次登录的时候需要运行 <code>/opt/sw/bin/pathsetup.command</code> 命令。你可以通过删除下面的文件 <code>~/Library/Preferences/com.apple.Terminal.plist</code> 来修正这一点。</p><p>如果这个配置文件里面有你需要保留的配置信息（所以你不能删除它），你可以用纯文本编辑器来编辑它，删除包含 <code>/opt/sw/bin/pathsetup.command</code> 的一行。</p></div>
    </a>
    <a name="ext-drive">
...
#     expected: 'nobody'</pre><p>那么你需要在 Fink 安装的驱动器/分区上运行 <b>Get Info</b>，并取消选择 "Ignore ownership" 的按钮。</p></div>
    ...
    <a name="cant-move-fink">
      <div class="question"><p><b><?php echo FINK_Q ; ?>5.30: I can't update Fink, because it can't move /opt/sw/fink out of the way.</b></p></div>
      <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> This error:</p><pre>Failed: Can't move "/opt/sw/fink" out of the way.</pre><p>is usually due, in spite of what it says, to permissions errors in one of the temporary directories that get created during a <code>selfupdate</code>.  Remove these:</p><pre>sudo rm -rf /opt/sw/fink.tmp /opt/sw/fink.old</pre></div>
    </a>
