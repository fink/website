...
<h1>F.A.Q. - 6 Compile Problems - General</h1><a name="compiler"><div class="question"><p><b>Q6.1: A configure script complains that it can't find an "acceptable cc".
        What's that?</b></p></div><div class="answer"><p><b>A:</b> Read the docs next time. To compile packages from source, you must
        install the Developer Tools, which among other stuff contains the C
        compiler, <code>cc</code>.</p></div></a><a name="cvs"><div class="question"><p><b>Q6.2: When I try a "fink selfupdate-cvs" I get this message: "cvs:
        Command not found."</b></p></div><div class="answer"><p><b>A:</b> You need to install the Developer Tools.</p></div></a><a name="missing-make"><div class="question"><p><b>Q6.3: I'm getting an error message involving <code>make</code></b></p></div><div class="answer"><p><b>A:</b> if your message is of the form</p><pre>make: command not found</pre><p>or</p><pre>Can't exec "make": 
No such file or directory at /sw/lib/perl5/Fink/Services.pm line 190.</pre><p>It means you need to install the Developer Tools.</p><p>On the other hand, if your error message looks like</p><pre>make: illegal option -- C</pre><p>then you've replaced the GNU version of the <code>make</code>
        ...
        <code>make</code>.</p></div></a><a name="head"><div class="question"><p><b>Q6.4: I'm getting a strange usage message from the head command. What's
        broken?</b></p></div><div class="answer"><p><b>A:</b> If you're seeing this:</p><pre>Unknown option: 1 Usage: head [-options] &lt;url&gt;...</pre><p>followed by a list of option descriptions, you have a broken
        ...
        easier to solve: rename <code>/sw/bin/HEAD</code>.</p></div></a><a name="also_in"><div class="question"><p><b>Q6.5: When I try to install a package I get an error message about trying
        to overwrite a file that is in another package.</b></p></div><div class="answer"><p><b>A:</b> This occasionally happens with splitoff packages (i.e. the ones
        ...
        package that you are trying to install.</p></div></a><a name="weak_lib"><div class="question"><p><b>Q6.6: After I installed the December 2002 Development Tools I get
        messages about "weak libraries".</b></p></div><div class="answer"><p><b>A:</b> This is new with the December 2002 Tools. You may occasionally see
...
MACOSX_DEPLOYMENT_TARGET environment variable set to: 10.1</pre><p>You may regard these as harmless. If you are curious, read through
        ...
        startup or not, for applications that use weak references.</p></div></a><a name="mv-failed"><div class="question"><p><b>Q6.7: What does "execution of mv failed, exit code 1" mean when I try to
        build a package?</b></p></div><div class="answer"><p><b>A:</b> If you have StuffIt Pro installed, it could be that you have
...
Failed: installing foo-0.1.2-3 failed</pre><p>then you should look for <code>libbar</code> somewhere
        further back in the output of your build attempt.</p></div></a><a name="node-exists"><div class="question"><p><b>Q6.8: I can't install a package | update because I get a message that a
        "node" already exists.</b></p></div><div class="answer"><p><b>A:</b> These errors look something like this:</p><pre>Failed: Internal error: node for system-xfree86 already exists</pre><p>This problem is that the dependency engine is confused, due to
        changes in some of the package info files. To fix it:</p><ul><li><p>Remove the offending package by force, e. g.</p>
          <pre>sudo dpkg -r --force-all system-xfree86</pre>
          <p>for the example given above.</p></li><li><p>Try again to install | upgrade. At some point a "virtual
          ...
          build.</p></li></ul></div></a><a name="usr-local-libs"><div class="question"><p><b>Q6.9: I've heard that libraries installed in /usr/local/lib sometimes
        cause build problems for Fink. Is this true?</b></p></div><div class="answer"><p><b>A:</b> This is a frequent source of problems, because the package
        ...
        back:</p><pre>sudo mv /usr/local.moved /usr/local</pre></div></a><a name="toc-out-of-date"><div class="question"><p><b>Q6.10: When I try to build a package, I get a message that a "table of
        contents" is out of date. What do I need to do?</b></p></div><div class="answer"><p><b>A:</b> The output hints at what to do. The message is usually something
...
rerun ranlib(1) (can't load from it)</pre><p>What you need to do is run ranlib (as root) on whatever library is
        ...
        run:</p><pre>sudo ranlib /sw/lib/libintl.a</pre></div></a><a name="fc-atlas"><div class="question"><p><b>Q6.11: Fink Commander hangs when I try to install atlas.</b></p></div><div class="answer"><p><b>A:</b> This happens because one of the steps in the build of
        ...
        instead.</p></div></a><a name="basic-headers"><div class="question"><p><b>Q6.12: I get messages saying that I'm missing stddef.h. Where do I find
        it?</b></p></div><div class="answer"><p><b>A:</b> This header, and many others, are provided by the DevSDK package of
        ...
        the DevSDK package using a Custom Install.</p></div></a><a name="multiple-dependencies"><div class="question"><p><b>Q6.13: I can't update, because Fink is "unable to resolve version conflict
        on multiple dependencies".</b></p></div><div class="answer"><p><b>A:</b> To get around this, try updating a single package, then try to use
        "fink update-all" again. If you still get the message, repeat the
        process.</p></div></a><a name="dpkg-parse-error"><div class="question"><p><b>Q6.14: I can't install anything because I get "dpkg: parse error, in file
        `/sw/var/lib/dpkg/status'"!</b></p></div><div class="answer"><p><b>A:</b> This means that somehow your dpkg database got corrupted, usually
        ...
        installed before the problem started occurring.</p></div></a><a name="freetype-problems"><div class="question"><p><b>Q6.15: I get errors involving freetype.</b></p></div><div class="answer"><p><b>A:</b> There are several varieties of such errors. If your error looks
...
undefined reference to _FT_Access_Frame</pre><p>then you probably have a binary version that built fine with
        ...
        via <code>sudo apt-get update ; sudo apt-get dist-upgrade</code>.</p></div></a><a name="dlfcn-from-oo"><div class="question"><p><b>Q6.16: I get build errors involving `Dl_info'.</b></p></div><div class="answer"><p><b>A:</b> If you have an error that looks like this</p><pre>unix_dl.c: In function `rep_open_dl_library':
...
sudo ln -s /usr/lib/libdl.dylib /usr/local/lib/libdl.dylib</pre></div></a><a name="gcc2"><div class="question"><p><b>Q6.17: Fink says I'm missing <code>gcc2</code> but I can't seem to
        install it.</b></p></div><div class="answer"><p><b>A:</b> This is because <code>gcc2</code> is a virtual package to
        ...
        part of their main Developer Tools installation.</p></div></a><a name="system-java"><div class="question"><p><b>Q6.18: Fink says <code>Failed: Can't resolve dependency "system-java14-dev"</code>, but there's no such package.</b></p></div><div class="answer"><p><b>A:</b> That's because it's a virtual package.  This type of error occurs when Java gets updated by Software Update:  the header files get removed, which causes the -dev package not to be generated.</p><p>You need to download the appropriate <code>Java Developer Tools</code> package from <a href="http://connect.apple.com">Apple</a>.  In this specific case that's the <code>Java 1.4.2 Developer Tools</code>.</p></div></a><p align="right">
