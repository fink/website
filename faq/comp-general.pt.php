...
<h1>Perguntas frequentes - 6. Problemas de Compilação - Geral</h1>
    ...
    <a name="compiler">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.1: Um script de configuração reclama que não consegue encontrar um
        "acceptable cc". O que é isso?</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Leia a documentação! Para compilar pacotes a partir do código fonte,
        você precisa instalar o Xcode que, dentre outros, contém o compilador
        C, <code>cc</code>.</p></div>
    </a>
    <a name="cvs">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.2: Quando tento executar o comando <code>fink selfupdate-cvs</code> eu
        recebo esta mensagem: "cvs: Command not found." ("cvs: Comando não
        encontrado").</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Você precisa instalar o Xcode.</p></div>
    </a>
    <a name="missing-make">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.3: Estou recebendo uma mensagem de erro envolvendo o
        <code>make</code>.</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Caso sua mensagem seja do formato</p><pre>make: command not found</pre><pre>(make: comando não encontrado)</pre><p>ou</p><pre>Can't exec "make": 
No such file or directory at /sw/lib/perl5/Fink/Services.pm line 190.</pre><p>Isso significa que você precisa instalar o Xcode.</p><p>Por outro lado, se sua mensagem de erro se parece com</p><pre>make: illegal option -- C</pre><pre>(make: opção ilegal -- C)</pre><p>então você substituiu a versão GNU do utilitário <code>make</code>
    ...
    <a name="head">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.4: Estou recebendo do comando head uma mensagem estranha. O que está
        errado?</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Caso esteja vendo isto:</p><pre>Unknown option: 1 Usage: head [-options] &lt;url&gt;...</pre><pre>(Opção desconhecida: 1 Uso: head [-opções] &lt;url&gt;...</pre><p>seguido por uma lista de descrições de opções, você tem um
    ...
    <a name="also_in">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.5: Quando tento instalar um pacote, recebo uma mensagem de erro sobre
        tentativa de sobrescrita de um arquivo que está em outro pacote.</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Isto acontece ocasionalmente com pacotes múltiplos (isto é, pacotes
...
rerun ranlib(1) (can't load from it)</pre><p>O que você precisa fazer é executar o comando ranlib como root em
    ...
    <a name="basic-headers">
      ...
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Ambos estes problemas são geralmente devidos à ausência de arquivos
    ...
    <a name="dpkg-parse-error">
      ...
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Isto significa que de alguma forma o seu banco de dados dpkg foi
        ...
        Você pode corrigir esta situação copiando uma versão anterior do banco
        de dados, como:</p><pre>sudo cp /sw/var/lib/dpkg/status-old /sw/var/lib/dpkg/status</pre><p>É possível que você precise reinstalar alguns dos últimos pacotes
...
sudo ln -s /usr/lib/libdl.dylib /usr/local/lib/libdl.dylib</pre></div>
    ...
    <a name="dpkg-split">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.18: Quando tento instalar qualquer pacote, recebo a mensagem <q>dpkg
        ...
        isto?</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Isto geralmente pode ser corrigido através da configuração correta
