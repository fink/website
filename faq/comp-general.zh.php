...
要从源代码编译软件包，你需要安装开发工具包，其中就包括 C 编译器：<code>cc</code>。
    ...
    <a name="missing-make">
      <div class="question"><p><b>Q6.3: 我碰到一个涉及 <code>make</code> 的错误。
        </b></p></div>
...
现在源代码发布版的 bootstrap 脚本会检查这一点，但你仍然会碰到这个问题，如果你使用二进制安装或在安装 Fink 以后又安装 libwww。
    ...
    <a name="also_in">
      ...
      <div class="answer"><p><b>A:</b> 当你安装那些具有版本分支的软件包（也就是那些名字末尾有 -dev，-shlibs，等的）时，如果需要有些文件从一个版本分支移动到另一个版本分支（例如，从 <code>foo</code> 到 <code>foo-shlibs</code>）的时候，你就会碰到这个错误。你可以做的是使用你现在安装的版本来覆盖旧有的版本（因为他们通常是相同的）：</p><pre>sudo dpkg -i --force-overwrite <b>文件名</b>
        </pre><p>这里文件名 <b>filename</b> 是对应于你要安装的软件包的 .deb 文件名称。</p></div>
    </a>
    <a name="weak_lib">
      <div class="question"><p><b>Q6.6: 我安装了 December 2002 开发工具包以后，碰到 "weak libraries" 的错误。</b></p></div>
      <div class="answer"><p><b>A:</b> 这是 December 2002 工具的新问题。偶尔你会看到这样的信息（比如说选择 libgdk-pixbuf）：</p><p>
    ...
    <a name="mv-failed">
      <div class="question"><p><b>Q6.7: 当我构建一个软件包时，"execution of mv failed, exit code 1" 错误是什么意思？</b></p></div>
      <div class="answer"><p><b>A:</b> 如果你安装了 StuffIt Pro，这可能是因为你使用了 "Archive Via Real Name" 模式。在系统预置中检查 StuffIt 面板，禁用 "ArchiveViaRealName"。它包括对一些重要的系统调用的不正确的替代，从而引起一些古怪的错误。</p><p>否则，一个 <code>mv</code> 错误通常意味着构建过程更早期发生了错误，但是构建过程并没有因为那个错误而终止。要找到发生的问题的文件，可以搜索那个不存在文件构建时候的输出。比如，如果你碰到这样的错误：</p><pre>mv /sw/src/root-foo-0.1.2-3/sw/lib/libbar*.dylib \
...
Failed: installing foo-0.1.2-3 failed</pre><p>那么你应该在你构建输出信息的更前面的地方寻找有关 <code>libbar</code> 的信息。</p></div>
    ...
    <a name="node-exists">
      <div class="question"><p><b>Q6.8: 我无法安装软件包或更新它，因为我碰到一个错误说一个 "node" 已经存在。</b></p></div>
    ...
    <a name="usr-local-libs">
      <div class="question"><p><b>Q6.9: 我听说安装在 /usr/local/lib 的库有时会引起 Fink 构建的问题。是这样吗？</b></p></div>
      <div class="answer"><p><b>A:</b> 这是一个经常发生的问题，因为软件包的配置脚本会在搜索 <code>/usr/local/lib</code> 之后才在 Fink 路径中搜索库。如果你碰到其它 FAQ 中没有解释的构建问题，你应该检查涉及的库时候在 <code>/usr/local/lib</code> 中。如果是这样的话，尝试把 <code>/usr/local</code> 改成其它名字，例如：</p><pre>sudo mv /usr/local /usr/local.moved</pre><p>完成你的构建，然后把 <code>/usr/local</code> 改回来：</p><pre>sudo mv /usr/local.moved /usr/local</pre></div>
    </a>
    <a name="toc-out-of-date">
      <div class="question"><p><b>Q6.10: 当我构建一个软件包的时候，我碰到一个消息说 "table of contents" 已经过时。我需要怎么办？</b></p></div>
    ...
    <a name="fc-atlas">
      <div class="question"><p><b>Q6.11: 当我安装 atlas 时 Fink Commander 挂了。</b></p></div>
      <div class="answer"><p><b>A:</b> 这原因时在构建 <code>atlas</code> 时，Fink Commander 遗漏了一个发向用户要求选择的信息。你需要使用 <code>fink install atlas</code> 命令来安装。</p></div>
    </a>
    <a name="basic-headers">
      <div class="question"><p><b>Q6.12: 我碰到信息说我缺少 stddef.h 文件。我可以在哪里找到它？</b></p></div>
      <div class="answer"><p><b>A:</b> 这个头文件，以及很多其它类似文件，都是由开发工具包中的 DevSDK 包提供。检查你系统中是否存在 <code>/Library/Receipts/DevSDK.pkg</code> 文件。如果没有的话，再运行一次开发工具安装程序，并使用定制安装来安装 DevSDK 包。</p></div>
    </a>
    <a name="multiple-dependencies">
      <div class="question"><p><b>Q6.13: 我无法升级，因为 Fink "unable to resolve version conflict on multiple dependencies"。</b></p></div>
      <div class="answer"><p><b>A:</b> 要回避这个问题，尝试升级一个单独的软件包，然后再次尝试使用 "fink update-all"。如果你还碰到这个信息，重复这个过程。</p></div>
    </a>
    <a name="dpkg-parse-error">
      <div class="question"><p><b>Q6.14: 我不能安装任何东西，因为我碰到 "dpkg: parse error, in file `/sw/var/lib/dpkg/status'" 错误！</b></p></div>
      <div class="answer"><p><b>A:</b> 这意味着你的 dpkg 数据库被损坏了，通常是由于一次程序崩溃或其它不可恢复错误造成。你可以用拷贝以前的版本的数据库办法来解决，象这样：</p><pre>
sudo cp /sw/var/lib/dpkg/status-old /sw/var/lib/dpkg/status
</pre><p>你也许需要重新安装问题发生前安装的最后几个软件包。</p></div>
    </a>
    <a name="freetype-problems">
      <div class="question"><p><b>Q6.15: 我碰到一个涉及 freetype 的错误。</b></p></div>
      <div class="answer"><p><b>A:</b> 这个错误有几种形式。如果你的错误是这样的：</p><pre>/sw/include/pango-1.0/pango/pangoft2.h:52: error: parse error before '*' token
...
/sw/lib/libqt-mt.3.dylib undefined reference to _FT_Access_Frame
    ...
    <a name="dlfcn-from-oo">
      <div class="question"><p><b>Q6.16: 我碰到一个涉及 `Dl_info' 的编译错误。</b></p></div>
