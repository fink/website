...
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Presque tous les fichiers installés par fink se trouvent dans le répertoire /opt/sw (ou bien l'endroit où vous avez choisi de faire l'installation). Donc, pour supprimer Fink, entrez cette commande :</p><pre>sudo rm -rf /opt/sw</pre><p>Les seules exceptions à cette règle concernent XFRee86 et X.org. Si vous avez installé un serveur X via Fink (vous avez, par exemple, installé le paquet <code>xfree86</code> ou <code>xfree86-rootless</code>, ou encore <code>xorg</code> au lieu d'utiliser <code>system-xfree86</code>) et que vous voulez le supprimer, vous devrez aussi saisir ceci :</p><pre>sudo rm -rf /usr/X11R6 /etc/X11 /Applications/XDarwin.app</pre><p>Si vous ne pensez pas réinstaller Fink, vous pouvez aussi supprimer, en utilisant un éditeur de texte, la ligne "<code>source /opt/sw/bin/init.csh</code>" que vous aviez ajoutée dans le fichier <code>.cshrc</code> ou la ligne "<code>source /opt/sw/bin/init.sh</code>" que vous aviez ajoutée dans le fichier <code>.bashrc</code>, selon vos réglages.</p></div>
</a>
<a name="bindist">
...
Updating using CVS failed. Check the error messages above.</pre><p>Dans ce cas-là, vous devez réinitialiser votre répertoire CVS. Pour ce faire, utilisez la commande :</p><pre>sudo find /opt/sw/fink -type d -name 'CVS' -exec rm -rf {}\
; fink selfupdate-cvs</pre><p>Si vous ne voyez aucun des messages ci-dessus, il est très probable que vous ayez modifié un fichier dans l'arborescence /opt/sw/fink/dists et que le mainteneur ait modifié ce fichier. Regardez de nouveau dans le message apparu après selfupdate-cvs les lignes commençant par "C", comme :</p><pre>C 10.2/unstable/main/finkinfo/libs/db31-3.1.17-6.info 
... (other info and patch files) ... 
### execution of su failed, exit code 1 
Failed: Updating using CVS failed. Check the error messages above.</pre><p>La lettre "C" signifie que CVS a rencontré un problème en essayant de mettre à jour la dernière version. La solution consiste à retirer tous les fichiers info ou patch correspondant aux lignes commençant par "C" dans le message de sortie de selfupdate-cvs, et à essayer de nouveau. Par exemple :</p><pre>sudo rm /opt/sw/fink/10.2/unstable/main/finkinfo/libs/db31-3.1.17-6.info
...
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Si cela se produit systématiquement, vous avez probablement modifié involontairement (ou omis de modifier) vos scripts de démarrage. Lancez le script <code>/opt/sw/bin/pathsetup.sh</code> (soit en double-cliquant dans le Finder, soit dans une fenêtre de terminal). Le script tentera de deviner votre shell par défaut et y ajoutera une commande de chargement du script d'initialisation du shell de Fink dans votre configuration shell. Vous devrez alors ouvrir une nouvelle session de terminal, de façon à ce que vos paramètres d'environnement soient pris en compte. <b>Note</b> : dans certaines versions anciennes de fink, ce script s'appelle <code>pathsetup.command</code> au lieu de <code>pathsetup.sh</code>. Pour modifier le script de démarrage, vous pouvez également lancer l'application <code>pathsetup.app</code>, qui se trouve dans l'image disque de la distribution binaire de Fink.</p><p>Par contre, si cela ne se produit que dans le terminal X11 d'Apple, la meilleure solution est de modifier la définition de "Terminal" dans le menu Applications via le sous-menu <b>Applications-&gt;Personnaliser le menu...</b>. Au lieu de :</p><pre>xterm</pre><p>mettez :</p><pre>xterm -ls</pre><p>dans le champ commande. Ici, <code>ls</code> signifie <q>login shell (c'est-à-dire shell d'ouverture de session)</q>. Il en résulte que tous les paramètres de votre ouverture de session sont utilisés (tout comme dans le Terminal de Mac OS X).</p><p>Les scripts <code>/opt/sw/bin/init.*</code> font beaucoup plus qu'ajouter le répertoire <code>/opt/sw/bin</code> à votre variable d'environnement PATH. La plupart des paquets ne fonctionnent pas correctement si ces scripts n'ont pas été lancés au démarrage.</p></div>
</a>
<a name="invisible-sw">
<div class="question"><p><b><?php echo FINK_Q ; ?>5.17: Est-il possible de masquer le répertoire /opt/sw dans le Finder pour éviter que les utilisateurs ne modifient les réglages de Fink ?</b></p></div>
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Oui. Si vous avez installé les Developer Tools, utilisez la commande suivante :</p><pre>sudo /Developer/Tools/SetFile -a V /opt/sw</pre><p>Cela a pour effet de rendre invisible le répertoire /opt/sw, tout comme le sont les autres répertoires standards utilisés par le système (/usr, etc...). Si vous n'avez pas installé les Developer Tools, il existe plusieurs applications de tierce-partie qui vous permettent de changer les attributs des fichiers - vous devez rendre le répertoire /opt/sw invisible.</p></div>
</a>
<a name="install-info-bad">
<div class="question"><p><b><?php echo FINK_Q ; ?>5.18: Il est impossible d'installer quoi que ce soit. Un message signale que l'option infodir n'est pas reconnue par la commande install-info (message en anglais "install-info: unrecognized option `--infodir=/opt/sw/share/info'"). Que faire ?</b></p></div>
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Ceci est généralement lié à un problème sur la variable d'environnement PATH. Lancez dans une fenêtre de terminal :</p><pre>printenv PATH</pre><p>Si <code>/opt/sw/sbin</code> n'apparaît pas, vous devrez alors modifier vos variables d'environnement comme expliqué dans les <a href="/doc/users-guide/install.php#setup">instructions</a> du Guide de l'Utilisateur. Si <code>/opt/sw/sbin</code> est bien là, mais qu'il y a d'autres répertoires avant (par exemple <code>/usr/local/bin</code>), vous devrez alors réordonner votre variable d'environnement PATH pour que <code>/opt/sw/sbin</code> soit proche du début. Si vous voulez cependant que les autres répertoires soit avant <code>/opt/sw/sbin</code>, et qu'ils contiennent des répertoires appelés eux aussi install-info, il faudra alors renommer temporairement ces sous-répertoires <code>install-info</code> quand vous utiliserez Fink.</p></div>
</a>
...
&gt; /opt/sw/var/lib/dpkg/info/<b>nom_du_paquet</b>.list</pre><p><b>Note</b> : les barres obliques inversées ont été rajoutées uniquement pour des raisons de formatage.</p><p>par exemple :</p><pre>dpkg -c /opt/sw/fink/debs/libgnomeui2-dev_2.0.6-2_darwin-powerpc.deb | \
awk \
'{if ($6 == "./") { print "/."; } \
else if (substr($6, length($6), 1) == "/") \
{print substr($6, 2, length($6) - 2); } \
else { print substr($6, 2, length($6) - 1);}}' \ 
&gt; /opt/sw/var/lib/dpkg/info/libgnomeui2-dev.list</pre><p><b>Note</b> : les barres obliques inversées ont été rajoutées uniquement pour des raisons de formatage.</p><p>Cela a pour effet d'extraire le contenu du fichier .deb, de tout supprimer sauf les noms de fichiers, puis de les écrire dans le fichier .list.</p></div>
</a>
...
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Les erreurs de la forme :</p><pre>gzip -dc /opt/sw/src/dpkg-1.10.9.tar.gz | /opt/sw/bin/tar -xf -
...
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Ce qui se passe ici est que, d'une façon ou d'une autre, l'application Terminal.app a été chargée d'exécuter <code>/opt/sw/bin/pathsetup.command</code> à chaque connexion. Vous pouvez corriger cela en supprimant le fichier de préférences, <code>~/Library/Preferences/com.apple.Terminal.plist</code>.</p><p>Si vous voulez conserver certaines préférences, vous pouvez modifier le fichier avec un éditeur de texte classique et supprimer la référence à <code>/opt/sw/bin/pathsetup.command</code>.</p></div>
</a>
<a name="ext-drive">
...
<div class="question"><p><b><?php echo FINK_Q ; ?>5.30: Il est impossible de mettre à jour Fink, car le répertoire /opt/sw/fink ne peut être déplacé. Que faire ?</b></p></div>
<div class="answer"><p><b><?php echo FINK_A ; ?>:</b> L'erreur suivante :</p><pre>Failed: Can't move "/opt/sw/fink" out of the way.</pre><p>est due, en général, à des permissions erronées dans un des répertoires temporaires créés durant l'exécution de <code>selfupdate</code>. Supprimez-les via la commande :</p><pre>sudo rm -rf /opt/sw/fink.tmp /opt/sw/fink.old</pre></div>
</a>
