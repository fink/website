...
$cvs_date = 'Date: 2004/05/29 01:24:15';
...
No such file or directory at /sw/lib/perl5/Fink/Services.pm line 190.</pre><p>cela signifie que vous devez installer les Developer Tools.</p><p>Si, par contre, le message est du type :</p><pre>make: illegal option -- C</pre><p>cela signifie que vous avez remplacé la version GNU de l'utilitaire <code>make</code> installée par les Developer Tools par une version BSD de make. De nombreux paquets utilisent des fonctionnalités spéciales implémentées seulement dans GNU make. Vérifiez que <code>/usr/bin/make</code> est un lien symbolique vers <code>gnumake</code>, et non vers <code>bsdmake</code>. De plus, assurez-vous que <code>/usr/local/bin/</code> ne contient aucune autre copie de <code>make</code>.</p></div>
    ...
    <a name="also_in">
      <div class="question"><p><b>Q6.5: À l'installation d'un paquet, un message d'erreur signale qu'il y a tentative d'écrasement d'un fichier situé dans un autre paquet.</b></p></div>
    ...
    <a name="weak_lib">
      <div class="question"><p><b>Q6.6: Après installation des outils de développement de décembre 2002, des messages concernant des "weak libraries" apparaissent.</b></p></div>
      <div class="answer"><p><b>A:</b> C'est un problème qui est apparu avec les outils de développement de décembre 2002. Parfois, des messages semblables au suivant apparaissent (on a choisi ici libgdk-pixbuf à titre d'exemple) :</p><pre>ld: warning dynamic shared library:
...
MACOSX_DEPLOYMENT_TARGET environment variable set to: 10.1</pre><p>Vous pouvez considérer ces messages comme inoffensifs. Si vous êtes du genre curieux, lisez les notes de mise à jour (release notes) qui se trouvent dans le répertoire de documentation du développeur, en particulier celles concernant GCC  et l'éditeur de liens, pour de plus amples informations. Cela se rapporte essentiellement à la façon dont sont considérés les symboles manquants lors de l'exécution d'applications qui utilisent des références faibles : erreur fatale ou non au lancement.</p></div>
    ...
    <a name="mv-failed">
      <div class="question"><p><b>Q6.7: Lors de la construction d'un paquet, le message suivant apparaît : "execution of mv failed, exit code 1"</b></p></div>
...
Failed: installing foo-0.1.2-3 failed</pre><p>vous devez rechercher <code>libbar</code> parmi les messages de sortie précédents du processus de construction.</p></div>
    ...
    <a name="node-exists">
      <div class="question"><p><b>Q6.8: Impossible d'installer ou de mettre à jour un paquet, un message indique qu'un "node" existe déjà.</b></p></div>
    ...
    <a name="toc-out-of-date">
      <div class="question"><p><b>Q6.10: Lors de la construction d'un paquet, un message indique que "table of
        contents" n'est pas à jour (out of date).</b></p></div>
      <div class="answer"><p><b>A:</b> Le message de sortie vous indique ce qu'il faut faire. En général, il est de la forme suivante :</p><pre>ld: table of contents for archive: 
...
rerun ranlib(1) (can't load from it)</pre><p>Vous devez exécuter ranlib (en tant que super-utilisateur) sur la librairie qui est la cause du problème. Par exemple, dans le cas ci-dessus, vous devez exécuter :</p><pre>sudo ranlib /sw/lib/libintl.a</pre></div>
    ...
    <a name="fc-atlas">
      <div class="question"><p><b>Q6.11: Fink Commander se bloque quand on tente d'installer atlas.</b></p></div>
    ...
    <a name="dpkg-parse-error">
      <div class="question"><p><b>Q6.14: Impossible d'installer quoi que ce soit, le message suivant apparaît : "dpkg: parse error, in file `/sw/var/lib/dpkg/status'"</b></p></div>
    ...
    <a name="freetype-problems">
      <div class="question"><p><b>Q6.15: Messages d'erreurs concernant freetype.</b></p></div>
      <div class="answer"><p><b>A:</b> Il y a plusieurs types d'erreurs concernant freetype. Si le message d'erreur ressemble au suivant : </p><pre>/sw/include/pango-1.0/pango/pangoft2.h:52: 
...
undefined reference to _FT_Access_Frame</pre><p>vous êtes probablement en présence d'une version binaire qui compile correctement avec <code>gcc3.3</code> sous Jaguar, mais pas sous Panther. Ce problème a été résolu, il suffit que vous mettiez à jour vos paquets, par exemple via <code>sudo apt-get update ; sudo apt-get dist-upgrade</code>.</p></div>
    ...
    <a name="dlfcn-from-oo">
      <div class="question"><p><b>Q6.16: Messages d'erreur concernant `Dl_info'.</b></p></div>
...
make[1]: *** [unix_dl.lo] Error 1</pre><p>vous avez certainement un header <code>/usr/local/include/dlfcn.h</code>, incompatible avec Panther. Déplacez-le.</p><p>Ce header est, en général, installé par Open Office, et vous devez le remplacer, de même que la librairie <code>/usr/local/lib/libdl.dylib</code>, par des liens symboliques vers les fichiers inclus dans Panther :</p><pre>sudo ln -s /usr/include/dlfcn.h /usr/local/include/dlfcn.h
sudo ln -s /usr/lib/libdl.dylib /usr/local/lib/libdl.dylib</pre></div>
    </a>
    <a name="gcc2">
      <div class="question"><p><b>Q6.17: Fink signale que <code>gcc2</code> n'existe pas, mais il ne semble pas possible de l'installer.</b></p></div>
      <div class="answer"><p><b>A:</b> <code>gcc2</code> est un paquet virtuel qui indique la présence de gcc-2.95 dans votre système. Installez le paquet gcc2.95 à partir de XCode Tools (les versions précédentes du système opératoire inclut gcc-2.95 dans l'installation standard des Developer Tools).</p></div>
    </a>
    <a name="system-java">
      <div class="question"><p><b>Q6.18: Fink signale <code>Failed: Can't resolve dependency "system-java14-dev"</code>, mais il n'existe pas de paquet system-java14-dev.</b></p></div>
