...
No such file or directory at /opt/sw/lib/perl5/Fink/Services.pm line 190.</pre><p>cela signifie que vous devez installer les Developer Tools.</p><p>Si, par contre, le message est du type :</p><pre>make: illegal option -- C</pre><p>cela signifie que vous avez remplacé la version GNU de l'utilitaire <code>make</code> installée par les Developer Tools par une version BSD de make. De nombreux paquets utilisent des fonctionnalités spéciales implémentées seulement dans GNU make. Vérifiez que <code>/usr/bin/make</code> est un lien symbolique vers <code>gnumake</code>, et non vers <code>bsdmake</code>. De plus, assurez-vous que le répertoire <code>/usr/local/bin/</code> ne contient aucune autre copie de <code>make</code>.</p></div>
</a>
<a name="head">
...
rerun ranlib(1) (can't load from it)</pre><p>Vous devez exécuter ranlib (en tant que super-utilisateur) sur la bibliothèque qui est la cause du problème. Par exemple, dans le cas ci-dessus, vous devez exécuter :</p><pre>sudo ranlib /opt/sw/lib/libintl.a</pre></div>
</a>
<a name="fc-atlas">
...
<div class="question"><p><b><?php echo FINK_Q ; ?>6.13: Il est impossible d'installer quoi que ce soit, un message indique que "dpkg est incapable d'analyser la structure du fichier /opt/sw/var/lib/dpkg/status" (message en anglais : "dpkg: parse error, in file `/opt/sw/var/lib/dpkg/status'"). De quoi s'agit-il ?</b></p></div>
