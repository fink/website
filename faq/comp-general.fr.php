...
Failed: installing foo-0.1.2-3 failed</pre><p>vous devez rechercher <code>libbar</code> parmi les messages de sortie précédents du processus de construction.</p></div>
    ...
    <a name="toc-out-of-date">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.10: Lors de la construction d'un paquet, un message indique que "table of
...
rerun ranlib(1) (can't load from it)</pre><p>Vous devez exécuter ranlib (en tant que super-utilisateur) sur la librairie qui est la cause du problème. Par exemple, dans le cas ci-dessus, vous devez exécuter :</p><pre>sudo ranlib /sw/lib/libintl.a</pre></div>
    ...
    <a name="basic-headers">
      <div class="question"><p><b><? echo FINK_Q ; ?>6.12: Un message indique qu'il est impossible de trouver stddef.h,  ou wchar.h, ou crt.o</b></p></div>
      <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Ces headers, comme beaucoup d'autres, sont fourniq par le paquet DevSDK des Developer Tools. Vérifiez que <code>/Library/Receipts/DevSDK.pkg</code> existe dans votre système. Si ce n'est pas le cas, relancez l'installeur des outils de développement et installez le paquet DevSDK en choisissant Custom Install.</p></div>
    </a>
