...
<h1>Ч.З.В. - 8. Проблемы использования пакетов - Общие вопросы</h1>
        ...
        <a name="xlocale">
            ...
            <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Ничего страшного, просто это значит, что программа будет использовать
                    ...
                    в <a href="http://fink.sourceforge.net/doc/x11/trouble.php#locale"></a>.</p></div>
        </a>
        <a name="passwd">
        ...
        <a name="compile-myself">
            ...
            <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Когда вы самостоятельно что-нибудь компилируете вне Fink,
                    ...
                    инсталлированные при помощи Fink. Для пакета, использующего
                    стандартный процесс конфигурирования/создания, вам надо установить некоторые
                    переменные величины среды:</p><p>-tcsh-</p><pre>setenv CFLAGS -I/sw/include setenv LDFLAGS -L/sw/lib
                    setenv CXXFLAGS $CFLAGS setenv CPPFLAGS $CXXFLAGS setenv
                    ACLOCAL_FLAGS "-I /sw/share/aclocal" setenv PKG_CONFIG_PATH "/sw/lib/pkgconfig"</pre><p>-bash-</p><pre>export CFLAGS=-I/sw/include export LDFLAGS=-L/sw/lib
                    export CXXFLAGS=$CFLAGS export CPPFLAGS=$CXXFLAGS export
                    ACLOCAL_FLAGS="-I /sw/share/aclocal" export PKG_CONFIG_PATH="/sw/lib/pkgconfig"</pre><p>Зачастую легче всего просто добавить их к своим файлам инициализации
                    (н-р <code>.cshrc</code> |
        ...
        <a name="x-options">
            ...
            <div class="answer"><p><b><? echo FINK_A ; ?>:</b> Это варианты в XFree86 (основанные на коде XFree8),
                    ...
                    см. в документе <a href="http://fink.sourceforge.net/doc/x11/index.php">Выполнение
                         X11</a>.</p></div>
        </a>
        <a name="no-display">
