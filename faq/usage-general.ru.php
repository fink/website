...
setenv MACOSX_DEPLOYMENT_TARGET 10.9</pre><p>(assuming that the build system is running OS 10.9 or later)</p><p>It is often easiest just to add these to your startup files (e.g.
        ...
        <a name="apple-x11-applications-menu">
            ...
            <div class="answer"><p><b><?php echo FINK_A ; ?>:</b> Apple X11 не отслеживает настройки среды Fink,
                    ...
                    следующее:</p><pre>source /opt/sw/bin/init.sh ;</pre><p>Н-р, если вы хотите запустить GIMP, инсталлированный при помощи Fink, вставьте</p><pre>source /opt/sw/bin/init.sh ; gimp</pre><p>в облать Command вашей записи GIMP.</p><p>Можно также отредактировать ваш файл .xinitrc (в вашем каталоге пользователя)
                    и добавить:</p><pre>source /opt/sw/bin/init.sh</pre><p>после первой строки.</p></div>
        </a>
        <a name="x-options">
