...
<a name="xterm-error">
<div class="question"><p><b><? echo FINK_Q ; ?>9.5: Le lancement de xterm échoue avec un message signalant que l'éditeur de liens dynamique ne trouve pas le symbole _tgetent défini dans xterm et censé être dans la bibliothèque /usr/lib/libSystem.B.dylib (message en anglais : "dyld: xterm Undefined symbols: xterm undefined reference to _tgetent expected to be defined in /usr/lib/libSystem.B.dylib"). Que se passe-t-il ?</b></p></div>
...
<a name="libXmuu">
<div class="question"><p><b><? echo FINK_Q ; ?>9.6: Au démarrage de XFree86, un message signale que l'éditeur de liens dynamique ne peut ouvrir la bibliothèque /usr/X11R6/lib/libXmuu.1.dylib ou la bibliothèque /usr/X11R6/lib/libXext.6.dylib appelées par xinit (message en anglais : "dyld: xinit can't open library: /usr/X11R6/lib/libXmuu.1.dylib" ou "dyld: xinit can't open library: /usr/X11R6/lib/libXext.6.dylib"). Que faire ?</b></p></div>
...
<p>Si vous utilisez une version récente de <code>fink</code> (c'est-à-dire &gt;= 0.18.3-1), la solution consiste, en général, à réinstaller le paquet X11User, car l'installeur omet parfois d'installer certains fichiers. Vous devrez peut-être effectuer la réinstallation plusieurs fois. L'exécution de la commande :</p>
...
<p>Si vous utilisez une version antérieure du paquet <code>fink</code>, il suffit, en général, de mettre à jour <code>fink</code>, par exemple via la commande :</p>
<pre>sudo apt-get update
sudo apt-get install fink</pre>
</li>
<li>
<b>Vous faites une installation à partir du source :</b>
<p>Si vous utilisez une version récente de <code>fink</code>, cette erreur signifie, en général, que vous devez installer ou réinstaller le paquet X11SDK, qui est <b>obligatoire</b> pour compiler les paquets à partir des sources. Vous le trouverez sur le CD XCode de Panther ou dans le répertoire XCode Tools du DVD de Tiger. Il <b>n'</b>est <b>pas</b> installé par défaut quand on installe XCode sous Panther. Il doit être soit installé en personnalisant l'installation de XCode, soit en double-cliquant sur le fichier X11SDK.pkg, situé dans le répertoire <code>Packages</code> du CD XCode. Par contre, il <b>est</b> installé par défaut quand on installe XCode sous Tiger (même si le paquet X11User n'est pas installé), mais il est possible que l'installeur oublie un fichier.</p>
...
<p>Si le problème persiste, exécutez la commande :</p>
<pre>fink list -i system-xfree86</pre>
<p>Cela devrait faire apparaître l'installation effective des paquets <code>system-xfree86</code>, <code>system-xfree86-shlibs</code> et <code>system-xfree86-dev</code>. Si le paquet <code>-dev</code> n'apparaît pas, réinstallez le paquet X11SDK, car il arrive que l'installeur d'Apple omette des fichiers. Il se peut que vous deviez faire la réinstallation plusieurs fois. Si l'un des deux autres paquets n'apparaît pas, réinstallez le paquet X11User (pour les mêmes raisons).</p>
...
<p>Si vous utilisez une version de <code>fink</code> antérieure à la version 0.17, vous devez mettre à jour <code>fink</code>, par exemple via la commande :</p>
