<?
  ...
  $languages = get_language_stats($conn,$from,$to,$sort,$filter);
  ...
  for ($i = 0; $i < $count; $i++) {
    $total_users += $languages[$i]["users"];
    $total_views += $languages[$i]["views"];
    $max_users = max($languages[$i]["users"],$max_users);
    $max_views = max($languages[$i]["views"],$max_views);
  }
  ...
  echo "<tr><td><h1>$string005</h1></td><td align=\"right\">\n";
  ...
  echo "<table border=\"0\" cellspacing=\"0\" width=\"730\" cellpadding=\"2\" align=\"center\">\n";
  echo "<tr>\n";
  echo "<td class=\"title\"><a href=\"lang.php?sort=0&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string022</a></td>\n";
  echo "<td class=\"title\"><a href=\"lang.php?sort=1&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string011</a></td>\n";
  echo "<td class=\"title\"><a href=\"lang.php?sort=2&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string012</a></td>\n";
  echo "</tr>\n";
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    echo "<td class=\"subtitle\" width=\"100\">" . $languages[$i]["name"] . " " . $languages[$i]["version"] . "</td>\n";
    echo "<td class=\"value\" width=\"315\"><img width=\"" . max(round(190 * $languages[$i]["users"]/$max_users,0),1) . "\" height=\"8\" src=\"imgs/blue.gif\"> " . $languages[$i]["users"] . " (" . round($languages[$i]["users"] / $total_users * 100,2) . "%) </td>\n";
    echo "<td class=\"value\" width=\"315\"><img width=\"" . max(round(190 * $languages[$i]["views"]/$max_views,0),1) . "\" height=\"8\" src=\"imgs/red.gif\"> " . $languages[$i]["views"] . " (" . round($languages[$i]["views"] / $total_views * 100,2) . "%) </td>\n";
    echo "</tr>\n";
  }
