<?
  ...
  $os = get_os_stats($conn,$from,$to,$sort,$filter);
  ...
  for ($i = 0; $i < $count; $i++) {
    $total_users += $os[$i]["users"];
    $total_views += $os[$i]["views"];
    $max_users = max($os[$i]["users"],$max_users);
    $max_views = max($os[$i]["views"],$max_views);
  }
  ...
  echo "<tr><td><h1>$string007</h1></td><td align=\"right\">\n";
  
  echo "<form action=\"os.php\" method=\"post\">\n";
  ...
  echo "<table border=\"0\" cellspacing=\"0\" width=\"730\" cellpadding=\"2\" align=\"center\">\n";
  echo "<tr>\n";
  echo "<td class=\"title\"><a href=\"os.php?filter=${filter}&amp;sort=0&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string024</a></td>\n";
  echo "<td class=\"title\"><a href=\"os.php?filter=${filter}&amp;sort=1&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string011</a></td>\n";
  echo "<td class=\"title\"><a href=\"os.php?filter=${filter}&amp;sort=2&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string012</a></td>\n";
  echo "</tr>\n";
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    if ($os[$i]["url"]) echo "<td class=\"subtitle\" width=\"190\"><a href=\"http://" . $os[$i]["url"] . "\">" . $os[$i]["name"] . "</a></td>\n";
    else echo "<td class=\"subtitle\" width=\"190\">" . $os[$i]["name"] . "</td>\n";
    
    echo "<td class=\"value\" width=\"270\"><img width=\"" . max(round(170 * $os[$i]["users"]/$max_users,0),1) . "\" height=\"8\" src=\"imgs/blue.gif\"> " . $os[$i]["users"] . " (" . round($os[$i]["users"] / $total_users * 100,2) . "%) </td>\n";
    echo "<td class=\"value\" width=\"270\"><img width=\"" . max(round(170 * $os[$i]["views"]/$max_views,0),1) . "\" height=\"8\" src=\"imgs/red.gif\"> " . $os[$i]["views"] . " (" . round($os[$i]["views"] / $total_views * 100,2) . "%) </td>\n";
    echo "</tr>\n";
  }
