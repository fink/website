<?
  ...
  else if (isset($_GET["filter"]) && ($_GET["filter"] == (int)$_GET["filter"])) $filter = $_GET["filter"];
  else $filter = 0;
  ...
  $archs = get_arch_stats($conn,$from,$to,$sort,$filter);
  ...
  for ($i = 0; $i < $count; $i++) {
    $total_users += $archs[$i]["users"];
    $total_views += $archs[$i]["views"];
    $max_users = max($archs[$i]["users"],$max_users);
    $max_views = max($archs[$i]["views"],$max_views);
  }
  ...
  echo "<tr><td><h1>$string001</h1></td><td align=\"right\">\n";
  ...
  echo "<table border=\"0\" cellspacing=\"0\" width=\"730\" cellpadding=\"2\" align=\"center\">\n";
  echo "<tr>\n";
  echo "<td class=\"title\"><a href=\"arch.php?filter=${filter}&amp;sort=0&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string010</a></td>\n";
  echo "<td class=\"title\"><a href=\"arch.php?filter=${filter}&amp;sort=1&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string011</a></td>\n";
  echo "<td class=\"title\"><a href=\"arch.php?filter=${filter}&amp;sort=2&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string012</a></td>\n";
  echo "</tr>\n";
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    echo "<td class=\"subtitle\" width=\"190\">" . $archs[$i]["name"] . "</td>\n";

    echo "<td class=\"value\" width=\"270\"><img width=\"" . max(round(160 * $archs[$i]["users"]/$max_users,0),1) . "\" height=\"8\" src=\"imgs/blue.gif\"> " . $archs[$i]["users"] . " (" . round($archs[$i]["users"] / $total_users * 100,2) . "%) </td>\n";
    echo "<td class=\"value\" width=\"270\"><img width=\"" . max(round(160 * $archs[$i]["views"]/$max_views,0),1) . "\" height=\"8\" src=\"imgs/red.gif\"> " . $archs[$i]["views"] . " (" . round($archs[$i]["views"] / $total_views * 100,2) . "%) </td>\n";
