<?
  ...
  $referers = get_referer_stats($conn,$from,$to,false,$sort,$filter);
  ...
  echo "<form action=\"referer.php\" method=\"post\">\n";
  ...
  echo "<table border=\"0\" cellspacing=\"0\" width=\"730\" cellpadding=\"2\" align=\"center\">\n";
  echo "<tr>\n";
  echo "<td class=\"title\"><a href=\"referer.php?filter=${filter}&amp;sort=0&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string028</a></td>\n";
  echo "<td class=\"title\"><a href=\"referer.php?filter=${filter}&amp;sort=1&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string011</a></td>\n";
  echo "<td class=\"title\"><a href=\"referer.php?filter=${filter}&amp;sort=2&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "\">$string012</a></td>\n";
  echo "</tr>\n";
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    if ($referers[$i]["name"] != "empty") echo "<td class=\"subtitle\" width=\"100\"><a href=\"refdetail.php?sort=${sort}&amp;datefrom=" . urlencode($datefrom) . "&amp;timefrom=" . urlencode($timefrom) . "&amp;dateto=" . urlencode($dateto) . "&amp;timeto=" . urlencode($timeto) . "&amp;referer=" . urlencode($referers[$i]["name"]) . "\">" . $referers[$i]["name"] . "</a></td>\n";
    else echo "<td class=\"subtitle\" width=\"100\">" . $referers[$i]["name"] . "</td>\n";
    
    echo "<td class=\"value\" width=\"315\"><img width=\"" . max(round(190 * $referers[$i]["users"]/$max_users,0),1) . "\" height=\"8\" src=\"imgs/blue.gif\"> " . $referers[$i]["users"] . " (" . round($referers[$i]["users"] / $total_users * 100,2) . "%) </td>\n";
    echo "<td class=\"value\" width=\"315\"><img width=\"" . max(round(190 * $referers[$i]["views"]/$max_views,0),1) . "\" height=\"8\" src=\"imgs/red.gif\"> " . $referers[$i]["views"] . " (" . round($referers[$i]["views"] / $total_views * 100,2) . "%) </td>\n";
    echo "</tr>\n";
  }
